# HongjianMa的学习笔记

> 欢迎来到马神的知识仓库：这里不保证每一篇都像论文一样严肃，但保证每一篇都在认真变强。

这里收着我的科研阅读、推荐系统、深度学习、算法与保研面试笔记。平时想到哪、学到哪、踩到坑了也记到哪——主打一个“脑子负责探索，博客负责别让我忘”。

## 怎么逛

- 想按主题看：点左侧目录，文件夹都可以展开/收起。
- 想直接找东西：用左上角搜索框，关键词一敲，知识自己出来。
- 想看最新整理：通常新笔记会自动出现在下面的目录里，不用手动翻 GitHub。

## 全部笔记

- 推荐系统
  - 多模态推荐
    - [【论文阅读】DimCL: Dimension-Aware Augmentation in Contrastive Learning for Recommendation](<notes/1-推荐系统/1-多模态推荐/【论文阅读】DimCL Dimension-Aware Augmentation in Contrastive Learning for Recommendation.md>)
    - [【论文阅读】Dual-layer cross-modal alignment recommendation based on the diffusion model](<notes/1-推荐系统/1-多模态推荐/【论文阅读】Dual-layer cross-modal alignment recommendation based on the diffusion model.md>)
    - [【论文阅读】FITMM: adaptive frequency-aware multimodal recommendation via information-theoretic representation learning](<notes/1-推荐系统/1-多模态推荐/【论文阅读】FITMM adaptive frequency-aware multimodal recommendation via information-theoretic representation learning.md>)
    - [【论文阅读】Hypercomplex Prompt-aware Multimodal Recommendation](<notes/1-推荐系统/1-多模态推荐/【论文阅读】Hypercomplex Prompt-aware Multimodal Recommendation.md>)
    - [【论文阅读】IGDMRec: Behavior Conditioned Item Graph Diffusion for  Multimodal Recommendation](<notes/1-推荐系统/1-多模态推荐/【论文阅读】IGDMRec Behavior Conditioned Item Graph Diffusion for  Multimodal Recommendation.md>)
    - [【论文阅读】LETTER: Self-Harmonized Representation Learning  for Multimodal Recommendation](<notes/1-推荐系统/1-多模态推荐/【论文阅读】LETTER Self-Harmonized Representation Learning  for Multimodal Recommendation.md>)
    - [【论文阅读】MDVT: Enhancing Multimodal Recommendation with Model-Agnostic Multimodal-Driven Virtual Triplets](<notes/1-推荐系统/1-多模态推荐/【论文阅读】MDVT Enhancing Multimodal Recommendation with Model-Agnostic Multimodal-Driven Virtual Triplets.md>)
    - [【论文阅读】PEARL: A dual-layer graph learning for multimodal recommendation](<notes/1-推荐系统/1-多模态推荐/【论文阅读】PEARL A dual-layer graph learning for multimodal recommendation.md>)
    - [【论文阅读】Structured Spectral Reasoning for Frequency-Adaptive Multimodal Recommendation](<notes/1-推荐系统/1-多模态推荐/【论文阅读】Structured Spectral Reasoning for Frequency-Adaptive Multimodal Recommendation.md>)
    - [【论文阅读】TAMER: Interest Tree Augmented Modality Graph Recommender for Multimodal Recommendation](<notes/1-推荐系统/1-多模态推荐/【论文阅读】TAMER Interest Tree Augmented Modality Graph Recommender for Multimodal Recommendation.md>)
    - [【推荐系统论文】ITCoHD-MRec: An Independent Topological Preference-Aware and Cooperative Hypergraph Diffusion-Based Multimodal Recommender Model](<notes/1-推荐系统/1-多模态推荐/【推荐系统论文】ITCoHD-MRec An Independent Topological Preference-Aware and Cooperative Hypergraph Diffusion-Based Multimodal R.md>)
  - 序列推荐
    - [【SR】Time-Aware Adaptive Side Information Fusion for Sequential Recommendation](<notes/1-推荐系统/2-序列推荐/【SR】Time-Aware Adaptive Side Information Fusion for Sequential Recommendation.md>)
  - 好看的图和表
    - [好看的方法图](<notes/1-推荐系统/好看的图和表/好看的方法图.md>)
    - [好看的实验表](<notes/1-推荐系统/好看的图和表/好看的实验表.md>)
- C++算法
  - 力扣100
    - [最长连续序列](<notes/2-C++算法/1-力扣100/1-最长连续序列.md>)
    - [移动零](<notes/2-C++算法/1-力扣100/2-移动零.md>)
    - [盛最多水的容器](<notes/2-C++算法/1-力扣100/3-盛最多水的容器.md>)
    - [二叉树的中序遍历](<notes/2-C++算法/1-力扣100/4-二叉树的中序遍历.md>)
    - [翻转二叉树](<notes/2-C++算法/1-力扣100/5-翻转二叉树.md>)
    - [对称二叉树](<notes/2-C++算法/1-力扣100/6-对称二叉树.md>)
    - [二叉树的直径](<notes/2-C++算法/1-力扣100/7-二叉树的直径.md>)
    - [**108. 将有序数组转换为二叉搜索树**](<notes/2-C++算法/1-力扣100/8- 将有序数组转换为二叉搜索树.md>)
    - [验证二叉搜索树](<notes/2-C++算法/1-力扣100/9-验证二叉搜索树.md>)
    - [**105. 从前序与中序遍历序列构造二叉树**](<notes/2-C++算法/1-力扣100/10-从前序与中序遍历序列构造二叉树.md>)
    - [二叉树展开为链表](<notes/2-C++算法/1-力扣100/11-二叉树展开为链表.md>)
    - [路径总和III](<notes/2-C++算法/1-力扣100/12-路径总和III.md>)
    - [相交链表](<notes/2-C++算法/1-力扣100/13-相交链表.md>)
  - acwing
    - 动态规划
      - [数字组合](<notes/2-C++算法/2-acwing/1-动态规划/1-数字组合.md>)
  - 考研机试真题
    - [常见动态规划](<notes/2-C++算法/3-考研机试真题/1-常见动态规划.md>)
- 保研面试
  - 自己的论文
    - DuGRec
      - [DCT](<notes/3-保研面试/1-自己的论文/1-DuGRec/1-DCT.md>)
      - [方法流](<notes/3-保研面试/1-自己的论文/1-DuGRec/2-方法流.md>)
      - [三个对比学习细节](<notes/3-保研面试/1-自己的论文/1-DuGRec/3-三个对比学习细节.md>)
      - [item图的构建细节](<notes/3-保研面试/1-自己的论文/1-DuGRec/4-item图的构建细节.md>)
      - [频域构图KNN](<notes/3-保研面试/1-自己的论文/1-DuGRec/5-频域构图KNN.md>)
      - [user-item图重构](<notes/3-保研面试/1-自己的论文/1-DuGRec/6-user-item图重构.md>)
      - [EMA](<notes/3-保研面试/1-自己的论文/1-DuGRec/7-EMA.md>)
      - [实验部分](<notes/3-保研面试/1-自己的论文/1-DuGRec/8-实验部分.md>)
  - 推荐系统
    - [实验指标](<notes/3-保研面试/2-推荐系统/1-实验指标.md>)
  - 深度学习
    - [对比学习](<notes/3-保研面试/3-深度学习/1-对比学习.md>)
    - [快速傅里叶变换](<notes/3-保研面试/3-深度学习/2-快速傅里叶变换.md>)
  - 线性代数
    - [特征值和奇异值](<notes/3-保研面试/4-线性代数/1-特征值和奇异值.md>)
  - 计算机组成原理
    - [计算机系统的组成](<notes/3-保研面试/5-计算机组成原理/1-计算机系统的组成.md>)
    - [进制转换](<notes/3-保研面试/5-计算机组成原理/2-进制转换.md>)
    - [原码-补码-反码-移码](<notes/3-保研面试/5-计算机组成原理/3-原码-补码-反码-移码.md>)
    - [浮点数的表示](<notes/3-保研面试/5-计算机组成原理/4-浮点数的表示.md>)
    - [IEEE754浮点数标准](<notes/3-保研面试/5-计算机组成原理/5-IEEE754浮点数标准.md>)
    - [C语言中的数据类型及转换](<notes/3-保研面试/5-计算机组成原理/6-C语言中的数据类型及转换.md>)
    - [存储器的性能指标](<notes/3-保研面试/5-计算机组成原理/7-存储器的性能指标.md>)
    - [主存中数据的存放](<notes/3-保研面试/5-计算机组成原理/8-主存中数据的存放.md>)
    - [静态随机存取存储器SRAM](<notes/3-保研面试/5-计算机组成原理/9-静态随机存取存储器SRAM.md>)
    - [只读存储器ROM](<notes/3-保研面试/5-计算机组成原理/10-只读存储器ROM.md>)
    - [高速缓冲存储器cache](<notes/3-保研面试/5-计算机组成原理/11-高速缓冲存储器cache.md>)
    - [虚拟存储器](<notes/3-保研面试/5-计算机组成原理/12.虚拟存储器.md>)
    - [指令系统](<notes/3-保研面试/5-计算机组成原理/13-指令系统.md>)
    - [寻址方式](<notes/3-保研面试/5-计算机组成原理/14-寻址方式.md>)
    - [指令的操作类型](<notes/3-保研面试/5-计算机组成原理/15-指令的操作类型.md>)
    - [中央处理器](<notes/3-保研面试/5-计算机组成原理/16-中央处理器.md>)
  - 软件工程
    - [软件工程常见概念](<notes/3-保研面试/6-软件工程/1-软件工程常见概念.md>)
    - [黑盒测试和白盒测试](<notes/3-保研面试/6-软件工程/2-黑盒测试和白盒测试.md>)
    - [设计模式](<notes/3-保研面试/6-软件工程/3-设计模式.md>)
    - [MVC模式](<notes/3-保研面试/6-软件工程/4-MVC模式.md>)
    - [高内聚低耦合](<notes/3-保研面试/6-软件工程/5-高内聚低耦合.md>)
    - [软件生命周期](<notes/3-保研面试/6-软件工程/6-软件生命周期.md>)
    - [软件开发模型](<notes/3-保研面试/6-软件工程/7-软件开发模型.md>)
    - [需求工程](<notes/3-保研面试/6-软件工程/8-需求工程.md>)
    - [软件设计原则](<notes/3-保研面试/6-软件工程/9-软件设计原则.md>)
  - 算法设计与分析
    - [哈夫曼编码](<notes/3-保研面试/7-算法设计与分析/1-哈夫曼编码.md>)
    - [动态规划](<notes/3-保研面试/7-算法设计与分析/2-动态规划.md>)
    - [背包问题](<notes/3-保研面试/7-算法设计与分析/3-背包问题.md>)
  - 数据库
    - [数据库恢复技术](<notes/3-保研面试/8-数据库/1-数据库恢复技术.md>)
    - [关系数据库设计理论](<notes/3-保研面试/8-数据库/2-关系数据库设计理论.md>)
    - [E-R图](<notes/3-保研面试/8-数据库/3-E-R图.md>)
  - 计算机网络
    - [三种交换方式](<notes/3-保研面试/10-计算机网络/1-三种交换方式.md>)
    - [计算机网络的分类](<notes/3-保研面试/10-计算机网络/2-计算机网络的分类.md>)
    - [计算机网络的性能指标](<notes/3-保研面试/10-计算机网络/3-计算机网络的性能指标.md>)
    - [计算机网络体系结构](<notes/3-保研面试/10-计算机网络/4-计算机网络体系结构.md>)
    - [物理层的基本概念](<notes/3-保研面试/10-计算机网络/5-物理层的基本概念.md>)
    - [数据链路层概述](<notes/3-保研面试/10-计算机网络/6-数据链路层概述.md>)
    - [网络层](<notes/3-保研面试/10-计算机网络/7-网络层.md>)
    - [运输层](<notes/3-保研面试/10-计算机网络/8-运输层.md>)
    - [应用层](<notes/3-保研面试/10-计算机网络/9-应用层.md>)
  - 操作系统
    - [问题库](<notes/3-保研面试/11-操作系统/0-问题库.md>)
  - 英语口语
    - [常见日常问答](<notes/3-保研面试/12-英语口语/1-常见日常问答.md>)
  - 深度学习
    - [获取键向量维度，键向量的维度和值向量的维度相同](<notes/3-保研面试/13-深度学习/第二章 Transformer架构.md>)
- 手撕深度学习代码
  - Transformer
    - [自注意力](<notes/4-手撕深度学习代码/1-Transformer/1-自注意力.md>)
    - [多头注意力](<notes/4-手撕深度学习代码/1-Transformer/2-多头注意力.md>)
  - AI基础概念
    - [KL散度](<notes/4-手撕深度学习代码/2-AI基础概念/1-KL散度.md>)
    - [SafeSoftmax](<notes/4-手撕深度学习代码/2-AI基础概念/2-SafeSoftmax.md>)
  - 常见损失函数
    - [二分类交叉熵BCE](<notes/4-手撕深度学习代码/3-常见损失函数/1-二分类交叉熵BCE.md>)
    - [BPRLoss](<notes/4-手撕深度学习代码/3-常见损失函数/2-BPRLoss.md>)
    - [InfoNCE](<notes/4-手撕深度学习代码/3-常见损失函数/3-InfoNCE.md>)

---

慢慢看，不着急。知识这东西，今天多懂一点，明天就少慌一点。马神继续施工中。🚧
